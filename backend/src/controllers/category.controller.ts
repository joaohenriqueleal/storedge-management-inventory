...
export const createCategory = async (req: Request, res: Response) => {
  ...
  if (!/^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/.test(cor)) {
    return res
      .status(400)
      .json({ message: "Cor inválida. Use o formato hexadecimal: #RRGGBB ou #RGB." })
  }
...
export const deleteCategory = async (req: Request, res: Response) => {
  ...
  if (!nome) {
    return res
      .status(400)
      .json({ message: "Informe o nome da categoria para confirmar a exclusão." })
  }
  ...
  try {
    ...
    if (!category || category.userId !== req.userId || category.nome !== (nome as string)) {
      return res
        .status(404)
        .json({ message: "Categoria não encontrada, nome incorreto ou sem permissão." })
    }
