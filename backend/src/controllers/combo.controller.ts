...
export const createCombo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const categoriaPai = await prisma.categoria.findUnique({
      where: { id: produtoPai.categoriaId }
    })
    const categoriaFilho = await prisma.categoria.findUnique({
      where: { id: produtoFilho.categoriaId }
    })

    if (
      !categoriaPai ||
      categoriaPai.userId !== user.id ||
      !categoriaFilho ||
      categoriaFilho.userId !== user.id
    ) {
      return res
        .status(403)
        .json({ message: "Produtos do combo não pertencem ao usuário autenticado." })
    }
